...
export const VoiceChatBar: React.FC<VoiceChatBarProps> = ({ socket, roomCode, currentPlayer, room }) => {
  ...
  return (
    <div className="fixed bottom-4 left-1/2 -translate-x-1/2 z-50 glass-panel rounded-2xl px-4 py-2.5 shadow-2xl flex items-center gap-2 text-[11px] font-semibold max-w-[95vw]">
      {/* Player info */}
      <div className="flex items-center gap-2 border-r border-white/[0.06] pr-3">
        ...
        <span className="text-[#8a85a0] truncate max-w-[100px]">{currentPlayer?.name || 'Bạn'}</span>
      ...
      <div className="relative flex items-center">
        <button onClick={toggleMic} className={`flex items-center gap-1.5 px-3 py-1.5 transition-all ${isHost ? 'rounded-l-xl' : 'rounded-xl'} ${
          audioState.mic
          ...
          <span className="hidden sm:inline">{audioState.mic ? 'Bật' : 'Tắt'}</span>
          {isHost && <span className="text-[9px] text-amber-300/80 ml-0.5">({targetLabels[hostMicTarget]})</span>}
        </button>
        ...
        {isHost && showHostMicMenu && (
          <div className="absolute bottom-full mb-2 left-0 glass-panel rounded-xl shadow-2xl overflow-hidden z-50 w-48 p-1">
            ...
            {([
              ...
              <button key={opt.key} onClick={() => selectHostMicTarget(opt.key)}
                className={`w-full px-2.5 py-2 rounded-lg text-left text-[11px] font-semibold flex items-center gap-2 transition-colors ${
      ...
      {/* Living Speaker */}
      <button onClick={toggleLivingSpeaker} className={`flex items-center gap-1.5 px-3 py-1.5 rounded-xl transition-all ${
      ...
      {/* Status Badge */}
      <div onClick={initVoice} className={`flex items-center gap-1 px-2.5 py-1 rounded-full text-[10px] cursor-pointer hover:opacity-80 transition-all ${
        isConnected
        ...
        <span>{errorMsg ? `Lỗi: ${errorMsg}` : connectionStatus}</span>
      ...
      {isDead && (
        <div className="hidden md:flex items-center gap-1 px-2 py-1 bg-amber-500/[0.06] border border-amber-500/10 text-amber-300/80 rounded-lg text-[10px]">
